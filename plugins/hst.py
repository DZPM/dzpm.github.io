...
def link_icons(html, settings):
    ...
    def icon(name):
        ...
        if name not in _LINK_ICON:
            ...
            _LINK_ICON[name] = f'<svg class="link-icon" viewBox="0 0 24 24" aria-hidden="true">{inner}</svg> '
...
def style_feed(pelican):
    ...
    if xml.startswith(head) and "xml-stylesheet" not in xml:
        xml = head + f'\n<?xml-stylesheet type="text/xsl" href="{pelican.settings["SITEURL"]}/theme/feed.xsl"?>' + xml[len(head):]
        with open(path, "w", encoding="utf-8") as f:
            f.write(xml)
